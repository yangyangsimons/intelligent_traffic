...
export default function JunctionCapture() {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            window.removeEventListener('junctionImageDataChanged', handleImageDataChanged);
        };
    }, [dispatch]);
    ...
    const renderList = carData.map((item, index) => (
        <div className={styles.listItem} key={index}>
            ...
            <span className={styles.averageSpeed}>平均车速 {item.averageSpeed} km/h</span>
            <span className={styles.averageParking}>停车 {item.averageParking} 次</span>
        </div>
    ));
    ...
    const renderImages = images.map((img, index) => (
        <div className={styles.imgContainer} key={index}>
            ...
            <span className={styles.imgTitle}>{img.title} : {img.time}</span>
        </div>
    ));
